...
Policies are ordered by deployment priority and Policy 1 would 
...
legacy auth is permitted if an attacker can authenticate using 
...
internationally would be prompted for MFA and acceptable 
...
still be bypassed via legacy auth protocols by making the 
policy ineffective.
